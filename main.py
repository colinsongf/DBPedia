...
def print_place(results, target, metadata=None):
    used_labels = []
    for result in results["results"]["bindings"]:
        if result[target]["type"] == u"literal":
            if result[target]["xml:lang"] == "en":
                ...
                if label not in used_labels:
                    used_labels.append(label)
    used_labels.reverse()
    if used_labels.__len__>1:
        label = used_labels[0]+", "+used_labels[1]
    else:
        label = used_labels[0]
    print label
...
if __name__ == "__main__":
    ...
    questions = ["How old is Tony Blair?",  "How old is Johnny Depp",  "What is the birth place of David Cameron?", "What was the birth place of Alexandre Dumas?",
                "Where was Victor Hugo born?",    "What was the birth place of Alexandre Dumas?"              
                 ]
